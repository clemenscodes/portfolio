...
bun run nx generate @nx/next:component "$component" --directory="$directory" --project="$project" --export -s=css
rm "$lib/$css"
sed -i '1d' "$lib/$file"
sed -i 's/className={[^\}]*}//g' "$lib/$file"
bun run nx generate @nx/react:component-story --componentPath="$file" --project="$project"
bun run nx format
