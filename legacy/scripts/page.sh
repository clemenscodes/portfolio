...
bun run nx generate @nx/next:component "$page" --directory="$directory" --project=pages --export -s=css
rm "$lib/$css"
sed -i '1d' "$tsx"
sed -i 's/className={[^\}]*}//g' "$tsx"
bun run nx generate @nx/react:component-story --componentPath="$file" --project=pages
bun run nx format
