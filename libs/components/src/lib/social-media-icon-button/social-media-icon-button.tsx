...
export type SocialMediaIconButtonProps = React.ComponentPropsWithoutRef<'button'> & {
    linkProps: LinkProps;
...
export const SocialMediaIconButton: React.FC<SocialMediaIconButtonProps> = ({
    ...
    return (
        <button
            {...props}
            className={cn(
                'flex items-center justify-center rounded-full transition-all duration-200 ease-in hover:scale-125 md:hover:scale-150',
                className
            )}
        >
